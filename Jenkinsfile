pipeline {
    ...
    triggers {
        cron('H/30 * * * *') // Executa a cada 30 minutos
    }
    ...
    stages {
        ...
        stage('Run Script') {
            steps {
                withCredentials([
                    usernamePassword(credentialsId: 'anomali-creds',
                                     usernameVariable: 'ANOMALI_CREDS_USR',
                                     passwordVariable: 'ANOMALI_CREDS_PSW'),
                    string(credentialsId: 'halo-client-id', variable: 'HALO_CLIENT_ID'),
                    string(credentialsId: 'halo-client-secret', variable: 'HALO_CLIENT_SECRET')
                ]) {
                    sh '''
                        python3 threatstream-api.py threat_model_search "$ANOMALI_CREDS_USR" "$ANOMALI_CREDS_PSW" "$LAST_TIMESTAMP" "$HALO_CLIENT_ID" "$HALO_CLIENT_SECRET"
                    '''
                }
            }
        }
